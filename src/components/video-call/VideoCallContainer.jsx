...
function VideoCallContainer({role}) {
  ...
  const {room,doc,user } = useParams();

  console.log("GOT TRACKS!!",role,doc,user);


  const handleUserJoined = useCallback(({ email, id }) => {
    console.log("helloooooooooooooo",email ,id);
              ...
              !remoteSocketId ? (
                <div  >
                  <h1 className='text-lg font-bold font-mono my-5 text-center'>
                    {role==='reviewer'?'Waiting for Student to join the Room':'Waiting for Reviewer to join the Room'}
                  </h1>
