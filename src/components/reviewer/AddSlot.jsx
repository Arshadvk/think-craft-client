...
function AddSlot() {
    ...
    const validate = (values) => {
        ...
        if (timeDifference <= values.reviewTime) {
            errors.reviewTime = 'Review time must be less than the time difference between start and end times';
        }
    ...
    const handleSubmit = (values , { resetForm }) => {
        ...
        reviewerAxios.post('/add-slot' ,{values}).then(()=>{
            console.log("added");
            ...
            <section className="bg-gray-50 min-h-screen flex items-center justify-center">
                <div className="bg-[FFFFFF]-100-100 flex rounded-lg shadow-xl max-w-3xl p-5">
                    <div className="px-16">
                        ...
                        <Formik
                            initialValues={{ date: '', timeStart: '', timeEnd: '', reviewTime: '' }}
                            onSubmit={handleSubmit}
                            validate={validate}
                        >
                            <Form className="flex flex-col gap-1">
                                <label htmlFor="date" className="mt-4 text-sm font-semibold from-stone-500">
                                    Date
                                </label>
                                <Field type="date" name="date" className="p-2 ps-14 rounded-lg border w-full" min={getCurrentDate()} />
                                <ErrorMessage name="date" component="div" className="text-red-500 font-semibold text-sm" />

                                <label htmlFor="reviewTime" className="mt-1 text-sm font-semibold from-stone-500">Review Time (in minutes)</label>
                                <Field type="number" name="reviewTime" className="p-2 ps-14 rounded-lg border w-full" />
                                <ErrorMessage name="reviewTime" component="div" className="text-red-500 font-semibold text-sm" />
