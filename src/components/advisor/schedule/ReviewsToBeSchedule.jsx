...
function ReviewsToBeSchedule({ reviews }) {
  const reviewsData = reviews?.map((obj) => {
    // Create a new object with updated date property
    const newObj = {
      ...obj,
      reviews: [
        {
          ...obj?.reviews[0],
          date: moment(obj?.reviews[0]?.date).format('YYYY-MM-DD'), // Format the date
        },
      ],
    };
    return newObj;
  ...
  return (
    <div className='w-1/2 border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 shadow-2xl m-3 p-3'>
      ...
      {reviewsData?.map((obj, index) => {
        return (
          <div key={index} className='bg-gray-600 font-semibold text-sm p-2 rounded m-2' onClick={()=> handleSelectStudent(obj) }>
            <h1>
              { obj?.student?.name},{'week '+ obj?.student?.week},{obj?.student?.domain?.name} ,{obj?.reviews[0]?.date}
            </h1>
          </div>
        );
      })}
